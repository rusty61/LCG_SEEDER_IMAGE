...
// color.h should be included by the main .ino file or here if CUSTOM_COLOR_2 is used directly in create_alarm_button
// For now, create_alarm_button uses a generic grey, specific colors can be set after creation.
// #include "color.h"
...
// Implementation of the function to create a single alarm button
inline void create_alarm_button(lv_obj_t *parent, alarm_button_t *alarm_ui, const char *button_text, const char* name_text, lv_event_cb_t event_cb, void *user_data) {
    ...
    lv_obj_set_size(alarm_ui->btn, 160, 100);
    lv_obj_set_style_pad_all(alarm_ui->btn, 10, 0);
    lv_obj_set_style_border_width(alarm_ui->btn, 3, 0);
    lv_obj_set_style_border_color(alarm_ui->btn, lv_color_white(), 0);
    // Default button color - can be overridden after creation
    ...
    lv_label_set_text(alarm_ui->label, button_text);
    lv_obj_set_style_text_align(alarm_ui->label, LV_TEXT_ALIGN_CENTER, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_font(alarm_ui->label, &lv_font_montserrat_22, LV_PART_MAIN | LV_STATE_DEFAULT);
