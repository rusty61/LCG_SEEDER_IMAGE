...
static lv_obj_t * drill_status= NULL;
...
lv_obj_set_style_bg_color(scr, lv_color_black(), 0);
  ...
  // Drill down status
    //drill_status = lv_label_create(scr); // Declaration moved to global
    current_y_offset += label_spacing;
    drill_status = lv_label_create(scr);
    lv_obj_t * drill_img = lv_img_create(scr);                // create an image object
lv_img_set_src(drill_img, &img_drill_down);
lv_obj_set_width (drill_img, 140);        // 120px wide
lv_obj_set_height(drill_img, LV_SIZE_CONTENT);              // point it at your img_drill_down asset
lv_obj_align(drill_img, LV_ALIGN_CENTER, 0, 100);       // position it (e.g. 10px from top/left)  

    lv_label_set_text_fmt(drill_status, "Drill: UP");
    lv_obj_set_width(drill_status, 200);
    lv_obj_set_height(drill_status, LV_SIZE_CONTENT);
    lv_obj_set_style_pad_all(drill_status, 5, 0);
    lv_obj_set_style_border_width(drill_status, 1, 0);
    lv_obj_set_style_border_color(drill_status, lv_color_white(), 0);               // White border
    lv_obj_set_style_border_opa(drill_status, LV_OPA_COVER, 0);
    // New styling for drill_status background and text
    lv_obj_set_style_bg_color(drill_status, lv_palette_main(LV_PALETTE_YELLOW), 3); // Yellow background
    lv_obj_set_style_text_color(drill_status, lv_color_black(), 0);                 // Black text
    lv_obj_set_style_bg_opa(drill_status, LV_OPA_COVER, 0); // Ensure background is opaque
    lv_obj_align(drill_status, LV_ALIGN_CENTER, 0, 200);
    ...
    lv_obj_align(alarm_box, LV_ALIGN_CENTER, 0, 30);
 ...
 // LVGL task handler
    ...
    if (current_time - last_toggle_time >= toggle_interval) {
        ...
        if (test_mode_toggle_state) {
            ...
            if (drill_status) lv_label_set_text(drill_status, "Drill: DOWN");
                if (alarm_box) {
            ...
            if (drill_status) lv_label_set_text(drill_status, "Drill: UP");
                if (alarm_box) {
